 ...
 * Assignment:
 * 
 * Here I want you to be able to read a file which has titles of magazine articles and the page number they occur on in it.
 * I want you to ask the user for a page number, and give them the article which appears on that page number.
 * 
 * For example.
 * 
 *	example.txt
 ...
 * Algorithm:
 * 
 *	1. Make a struct for magazine contents with page number and article title
 *	2. Make functions to go with this struct (scan, print, compare, etc.)
 *	3. Ask user for input
 *	4. Compare user input against the "page number" part of the magazine contents struct
...
/*
 * scan_contents(): looks in a file and puts contents into the article_t type struct thing, then returns it
 */
...
int main (int argc, const char * argv[]) {
	...
	for (i = 1; i < MAX_ARTICLES; i++) { // go through array again, this time looking at what got put in in the previous loop
		...
		if (answer >= 1) {
			printf("\n Article %s starts on page %.0f. \n", *article[i].title, selected_page_number.two_digit_number);
			flag++;
		} 
		answer = 0; // reset
	}
